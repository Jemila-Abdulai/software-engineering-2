...
def remove_multiline_back_ticked_blocks(line_list): # remove the ```s and the lines between them; we don't want to evaluate comments in code
...
def error_message(message_content = "", message_prefix = "ERROR - ",):
  if isVerbose():
...
def remove_punctuation(lower_case_title):
  lower_case_title_no_punctuation = "".join(char for char in lower_case_title if char.isalnum() 
                                                                              or char == "-" 
                                                                              or char == " ")
...
def create_linker_half(line):
  ...
  try:
    ...
    error_message("There appears to have been an error here, likely not a good input like for creating a TOC entry for - skipping")
  return linker_half
...
def create_full_line(line):
  ...
  if title_half != None and linker_half != None:
    return title_half + linker_half
      ...
      if title_half == None:
        error_message("The title_half (create_full_line) has a None value, this is generated in a line that ends with \"#\" but doesn't have anything after that")
